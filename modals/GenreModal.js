...
import { Dimensions, View,StyleSheet,ActivityIndicator,Alert,Animated} from 'react-native';
...
export default function GenreModal({navigation,route}) {
    ...
    return(
        <View style={[styles.container,{width:'100%',position:'relative',backgroundColor:colors.mainBlackColor}]} >
            <ScreenHeaderWithFilter navigation={navigation} title={genreName} filter={true} filterValue={filterValue} handleFilterChange={handleFilterChange} />

            {isLoading?
                <SmallPosterLoadingContainer  />
                :
                <SmallPosterContainer navigation={navigation} type={type} handleReachEnd={handleReachEnd} movieData={movieData} pages={pages}  />
