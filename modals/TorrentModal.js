...
export default function TorrentModal({navigation,route}) {
    ...
    const getTorrents=async(page)=>{
        try {
            let response=await axios.get(`${Torrent_Search}${route.params.query}/${page}`)
            setTorrents((prev)=>{
                ...
                <View style={[styles.container,{backgroundColor:colors.mainBlackColor}]}>
                    {
                        ...
                        <SafeAreaView style={[styles.container,{backgroundColor:colors.mainBlackColor}]}>
                            <FlatList 
                                data={torrents}
                                keyExtractor={(item)=>item.magnet+Math.round(Math.random() * 10000000).toString()}
                                renderItem={({item})=>(
                                    <View key={item.magnet} style={s.torrentContainer}>
                                        ...
                                        <View style={[s.torrentDetailsContainer,s.flex]}>
                                            <View style={[s.torrentSizeContainer,s.flex]}>
                                                <FontAwesome name="database" size={22} color={colors.lightGray} />
                                                <Text style={s.normalText}>{item.size}</Text>
                                            </View>
                                            <View style={[s.torrentPeerContainer,s.flex]}>
                                                <MaterialIcons name="file-upload" size={22} color={colors.lightGray} />
                                            ...
                                            <View style={[s.flex]}>
                                                ...
                                                <TouchableOpacity onPress={()=>Linking.openURL(item.magnet)}>
                                                    <View style={[s.torrentLinkContainer,s.flex]}>
                                                        <MaterialCommunityIcons name="magnet" size={22} color={colors.lightGray} />
                                ...
                                ListFooterComponent={()=>(
                                    ((torrents.length / 20) + 1)<pages?<ActivityIndicator size='large' color={colors.mainBlue} />:null
                                )}
                                onEndReached={({distanceFromEnd})=>{
                                    if(!onEndReachedCalledDuringMomentum && ((torrents.length/20)+1)<pages  ){
                                        setRefreshing(true)
